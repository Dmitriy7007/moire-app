  ...
  <main class="content container">
    ...
    <div class="content__catalog">
      ...
      <section class="catalog">
        ...
        <div v-if="productsLoadingFailed" style="text-align: center">Произошла ошибка при загрузке товаров<br>
          <button class="cart__button button button--primery" style="width: 50%" @click.prevent="loadProducts()">Попробовать еще раз</button>
        </div>

        <ProductList/>

        <div  style="text-align: center; margin-bottom: 30px">
          <div style="margin-bottom: 10px">Изменить количество товаров отоброжаемых на странице:</div>
          <ul class="check-list">
            <BaseProductsInPage v-for="item in productsPerPage" :key="item.id" :item="item" v-model="currentProductsPerPage"/>
          </ul>
        </div>
...
<script>
  ...
  import BaseProductsInPage from '@/components/BaseProductsInPage.vue';
  ...
  export default {
    ...
    data() {
      return {
        productsPerPage: [{products:12, id:1}, {products:24, id:2}, {products:32, id:3}],
