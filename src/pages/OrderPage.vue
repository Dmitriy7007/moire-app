<template>
  ...
  <main
    ...
    v-else-if="$store.state.cartLoadingFailed"
    style="text-align: center"
  >
    Не удалось загрузить страницу оформления заказа<br />
    <button
      class="cart__button button button--primery"
      style="width: 30%"
      @click.prevent="cartLoad()"
    >
      Попробовать еще раз
    </button>
  </main>
      ...
      <form class="cart__form form" action="#" method="POST" @submit.prevent="order">
        <div class="cart__field">
          ...
          <div class="cart__options">
            <div class="form-error" v-if="formError.deliveryTypeId">
  ...
  methods: {
    ...
    order() {
      ...
      this.loadOrder(this.formData)
        .then(() => {
          this.$router.push({ name: "orderInfo", params: { id: this.$store.state.orderId } });
        })
        .catch((error) => {
